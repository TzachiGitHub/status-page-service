import { CheckStatus, ComponentStatus } from '../types/enums.js';
...
const COMPONENT_STATUS_SEVERITY = [
...
export function getStatusColor(status) {
    if (status in COMPONENT_STATUS_CONFIG) {
        return COMPONENT_STATUS_CONFIG[status].color;
    }
    if (status in MONITOR_STATUS_CONFIG) {
        return MONITOR_STATUS_CONFIG[status].color;
    }
    return '#6b7280';
}
/**
 ...
 */
export function uptimeBars(checks, days) {
    const now = new Date();
    const bars = [];
